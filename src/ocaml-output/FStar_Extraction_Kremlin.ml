...
let extend : env -> Prims.string -> Prims.bool -> env =
  fun env  ->
    fun x  ->
      fun is_mut  ->
        let uu___155_2960 = env  in
        {
          names = ({ pretty = x; mut = is_mut } :: (env.names));
          names_t = (uu___155_2960.names_t);
          module_name = (uu___155_2960.module_name)
        }
  
let extend_t : env -> Prims.string -> env =
  fun env  ->
    fun x  ->
      let uu___156_2969 = env  in
      {
        names = (uu___156_2969.names);
        names_t = (x :: (env.names_t));
        module_name = (uu___156_2969.module_name)
      }
    ...
    fun d  ->
      ...
      | FStar_Extraction_ML_Syntax.MLM_Let
          ...
          let assumed =
            FStar_Util.for_some
              (fun uu___152_3439  ->
                 match uu___152_3439 with
                 | FStar_Extraction_ML_Syntax.Assumed  -> true
                 | uu____3440 -> false) flags
             in
          let env1 =
          ...
          let env2 =
            FStar_List.fold_left
             ...
             in
          let rec find_return_type i uu___153_3454 =
            match uu___153_3454 with
      ...
      | FStar_Extraction_ML_Syntax.MLM_Let
          ...
          let assumed =
            FStar_Util.for_some
              (fun uu___152_3601  ->
                 match uu___152_3601 with
                 | FStar_Extraction_ML_Syntax.Assumed  -> true
                 | uu____3602 -> false) flags
             in
          let env1 =
            if flavor = FStar_Extraction_ML_Syntax.Rec
            then extend env name false
          ...
          let env2 =
            FStar_List.fold_left
              (fun env2  -> fun name1  -> extend_t env2 name1) env1 tvars
             in
          let rec find_return_type i uu___153_3616 =
            match uu___153_3616 with
    ...
    fun e  ->
      ...
      | FStar_Extraction_ML_Syntax.MLE_Let
          ...
          let is_mut =
            FStar_Util.for_some
              (fun uu___154_4608  ->
                 match uu___154_4608 with
                 | FStar_Extraction_ML_Syntax.Mutable  -> true
                 | uu____4609 -> false) flags
             in
          let uu____4610 =
            if is_mut
            then
      ...
      | FStar_Extraction_ML_Syntax.MLE_App
          ...
          let uu____5102 =
            let uu____5107 = FStar_Util.must (mk_width m)  in (uu____5107, c)
             in
      ...
      | FStar_Extraction_ML_Syntax.MLE_App
          ...
          when is_machine_int m ->
          let uu____5128 =
            let uu____5133 = FStar_Util.must (mk_width m)  in (uu____5133, c)
             in
