    ...
    fun lid  ->
      let x =
        FStar_Util.find_map g.gamma
          (fun uu___130_750  ->
             match uu___130_750 with
             | Fv (fv',x) when FStar_Syntax_Syntax.fv_eq_lid fv' lid ->
                 FStar_Pervasives_Native.Some x
             | uu____755 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____756 =
  ...
  fun g  ->
    fun bv  ->
      let x =
        FStar_Util.find_map g.gamma
          (fun uu___132_796  ->
             match uu___132_796 with
             | Bv (bv',r) when FStar_Syntax_Syntax.bv_eq bv bv' ->
                 FStar_Pervasives_Native.Some r
             | uu____801 -> FStar_Pervasives_Native.None)
         in
      match x with
      | FStar_Pervasives_Native.None  ->
          let uu____802 =
  ...
  fun g  ->
    fun a  ->
      fun mapped_to  ->
        ...
        let uu___134_922 = g  in
        {
          tcenv;
          gamma;
          tydefs = (uu___134_922.tydefs);
          type_names = (uu___134_922.type_names);
          currentModule = (uu___134_922.currentModule)
        }
    ...
    let cs' =
      FStar_List.fold_right
        (fun c  ->
           fun cs1  ->
             let uu____946 =
               let uu____949 = valid c  in
               if uu____949 then [c] else [95; 95]  in
             FStar_List.append uu____946 cs1) cs []
       in
    let cs'1 =
      match cs' with
        ...
        fun add_unit  ->
          fun is_rec  ->
            fun mk_unit  ->
              ...
              ((let uu___135_1204 = g  in
                {
                  tcenv;
                  gamma;
                  tydefs = (uu___135_1204.tydefs);
                  type_names = (uu___135_1204.type_names);
                  currentModule = (uu___135_1204.currentModule)
                }), mlident)
              ...
              then
                ...
                | (mlpath,mlsymbol) ->
                    ...
                    let gamma =
                      ...
                      :: (g.gamma)  in
                    ((let uu___136_1395 = g  in
                      {
                        tcenv = (uu___136_1395.tcenv);
                        gamma;
                        tydefs = (uu___136_1395.tydefs);
                        type_names = (uu___136_1395.type_names);
                        currentModule = (uu___136_1395.currentModule)
                      }), mlsymbol)
  ...
  fun g  ->
    fun fv  ->
      fun td  ->
        ...
        let uu___137_1492 = g  in
        {
          tcenv = (uu___137_1492.tcenv);
          gamma = (uu___137_1492.gamma);
          tydefs = ((m, td) :: (g.tydefs));
          type_names = (fv :: (g.type_names));
          currentModule = (uu___137_1492.currentModule)
        }
  
let extend_type_name : env -> FStar_Syntax_Syntax.fv -> env =
  fun g  ->
    fun fv  ->
      let uu___138_1509 = g  in
      {
        tcenv = (uu___138_1509.tcenv);
        gamma = (uu___138_1509.gamma);
        tydefs = (uu___138_1509.tydefs);
        type_names = (fv :: (g.type_names));
        currentModule = (uu___138_1509.currentModule)
      }
