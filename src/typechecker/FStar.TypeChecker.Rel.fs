...
let must_be_closed rng msg t =
    if not (Env.closed' t)
    then raise (Error (BU.format3 "Internal error: term is not closed (%s).\nt = (%s)\nFVs = (%s)\n"
                                  msg
                                  (Print.term_to_string t)
                                  (Free.names t |> BU.set_elements |> Print.bvs_to_string ", "), rng))
...
let p_scope prob =
   ...
   | (bv, _)::_ ->
       ...
       must_be_closed (p_loc prob) "p_scope" bv.sort;
       r
   end
...
let solve_prob' resolve_ok prob logical_guard uvis wl =
    ...
    let _ = match (Subst.compress uv).n with
        | Tm_uvar(uvar, k) ->
          ...
          must_be_closed (p_loc prob) "solve_prob'" phi;
          U.set_uvar uvar phi
...
and solve_t' (env:Env.env) (problem:tprob) (wl:worklist) : solution =
    ...
    let imitate orig (env:Env.env) (wl:worklist) (p:im_or_proj_t) : solution =
        ...
        must_be_closed (p_loc orig) "imitate" im;
                        ...
                        | Tm_uvar (uvar, _) ->
                          (match pat_vars env [] uv_args with
                           ...
                           | Some scope ->
                             ...
                             must_be_closed (p_loc orig) "solve_t_flex_rigid.subterms" uv_sol;
